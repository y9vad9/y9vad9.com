...
async function loadJsonLd(agents: Record<string, unknown> | undefined) {
  ...
  const config = { ...actual.config, agents, seo: { siteUrl: 'https://example.com' } }
  vi.doMock('~/site.config', () => ({ config }))
  // The builders take the resolved config as an argument now, so it is handed
  // back alongside them — the mock still matters for `resolveAgentsConfig` and
  // for `siteUrl()`, which do read the module.
  return { ...(await import('@lib/seo/jsonLd')), siteConfig: config }
}
...
describe('articleJsonLd — agent schema off (the default)', () => {
  it('emits none of the opt-in fields', async () => {
    const { articleJsonLd, siteConfig } = await loadJsonLd(undefined)
  ...
  it('still emits the baseline Article fields', async () => {
    const { articleJsonLd, siteConfig } = await loadJsonLd(undefined)
    const out = articleJsonLd(note(), 'en', siteConfig)
    expect(out['@type']).toBe('Article')
    expect(out.headline).toBe('Deep Modules')
  })

  it('returns null for definedTermJsonLd', async () => {
    const { definedTermJsonLd, siteConfig } = await loadJsonLd(undefined)
    expect(definedTermJsonLd(note(), 'en', siteConfig)).toBeNull()
  })
})

describe('articleJsonLd — agent schema on', () => {
  it('points `encoding` at the markdown mirror', async () => {
    const { articleJsonLd, siteConfig } = await loadJsonLd(ALL_ON)
    const out = articleJsonLd(note(), 'en', siteConfig) as Record<string, Record<string, string>>
  ...
  it('omits `encoding` for a noindex note, which has no mirror', async () => {
    const { articleJsonLd, siteConfig } = await loadJsonLd(ALL_ON)
    expect(articleJsonLd(note({ noindex: true }), 'en', siteConfig).encoding).toBeUndefined()
  })

  it('expresses a series as CreativeWorkSeries with position', async () => {
    const { articleJsonLd, siteConfig } = await loadJsonLd(ALL_ON)
    const out = articleJsonLd(note({ series: 'Kotlin Coroutines', order: 2 }), 'en', siteConfig, {
      seriesNotes: [
  ...
  it('builds a DefinedTerm inside the site glossary', async () => {
    const { definedTermJsonLd, siteConfig } = await loadJsonLd(ALL_ON)
    const out = definedTermJsonLd(note(), 'en', siteConfig)!
  ...
  it('adds knowsAbout to Person only when topics exist', async () => {
    const { personJsonLd, siteConfig } = await loadJsonLd(ALL_ON)
    expect(personJsonLd(siteConfig, ['kotlin', 'design']).knowsAbout).toEqual(['kotlin', 'design'])
    expect(personJsonLd(siteConfig, []).knowsAbout).toBeUndefined()
  })

  it('leaves knowsAbout off when its flag is off, even with topics', async () => {
    const { personJsonLd, siteConfig } = await loadJsonLd({ ...ALL_ON, schema: { knowsAbout: false } })
    expect(personJsonLd(siteConfig, ['kotlin']).knowsAbout).toBeUndefined()
  })
})
