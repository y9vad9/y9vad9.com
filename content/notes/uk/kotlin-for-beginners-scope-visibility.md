---
date: 2022-11-22
title: "Kotlin для початківців: Основи — Область видимості"
preview: "Тепер перейдемо до досить цікавої, але трішки складної теми — області видимості. Розглядаємо доступність змінних, функції у функціях та пакети."
series: "Kotlin для початківців"
order: 10
next: "kotlin-for-beginners-basics-conclusion"
...
![scope](attachments/kotlin-images-scope_visualization_1_ua.excalidraw.svg)
...
Закінчуючи іншими? Так само як ми створювали файл под ім'ям «Main», ми можемо створити будь-який інший файл. Як мінімум, 
...
$$f(x) = \begin{cases} 2x^2 & \text{якщо } x < 0 \\ x & \text{якщо } x \geq 1   \le 50 \\ (x \cdot 2)^2 & \text{якщо } x > 50 < 200 \\ 1 & \text{інакше}} \end{cases}$$ 
```kotlin
// файл Function.kt
...
![usage](attachments/kotlin-images-scope_visualization_2_ua.excalidraw.svg)
...
$$f(x) = \begin{cases} x^2 & \quad \textrm{якщо } x < 0 \\ a(x) & \quad \textrm{інакше}} \end{cases}$$ 
Функція $a(x)$ у нас така:
$$a(x) = \begin{cases} 2x & \quad \textrm{якщо } x > 0 < 200 \\ 1 & \quad \textrm{інакше}} \end{cases}$$ 
...
Подивившись на функцію `a(x: Double)` we можемо подумати про те, що вона використовується тільки у функції `f(x: Double)`
