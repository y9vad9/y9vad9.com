...
Ein Paket ist eines der ersten Konzepte, auf das du direkt nach dem Schreiben deines grundlegenden "Hello World"-Programms in Java oder Kotlin stößt. Die einfachste und doch irreführende Beschreibung ist einfach als Ordnerstruktur, die zur Organisation von Code und zur Vermeidung von Namenskonflikten verwendet wird.

Und obwohl dies teilweise stimmt, könnte es dir die falsche Perspektive geben, wann es tatsächlich verwendet werden sollte. Aber halten wir uns an eine Art Definition:
...
Dieser Ansatz kann zu einer fehlerhaften Denkweise bei der Gestaltung deiner eigenen Paketstruktur führen – du beginnst, in Bezug auf die Gruppierung von Dateien zu denken, anstatt aussagekräftige Namespaces und Grenzen zu definieren, was zu unklaren Verantwortlichkeiten führt. Das Ergebnis ist oft ein Verzeichnis voller Klassen, die nach Ort statt nach Zweck organisiert sind.

Auch wenn es dir ein gutes Gefühl gibt, dass die Dinge "organisiert" sind, führt es meist zu vielen Problemen. Dies ist das falsche Denkmodell.

## Pakete als Namespaces

Ein besseres Denkmodell ist es, Pakete als **semantische Grenzen** zu betrachten – sie sagen dir, welchen Teil des Systems du gerade betrachtest und idealerweise, wofür dieser Code verantwortlich ist.
...
Wenn du anfängst, Pakete als echte Namespaces zu behandeln, geschehen mehrere vorteilhafte Dinge:
- **Das Paket selbst wird Teil der Dokumentation** – allein durch den Blick darauf, wo eine Klasse lebt, bekommst du ein Gefühl dafür, was sie tut und wofür sie verantwortlich ist.
- **Verwandte Funktionalität bleibt natürlich zusammen**, während nicht verwandter Code getrennt bleibt. Es lässt dich auch mehr darüber nachdenken, "wofür ist es verantwortlich?" / "Wer ist der Besitzer dieser bestimmten Klasse/Funktion?".
...
Betrachte zum Beispiel ein großes Projekt, in dem mehrere Teams an benutzerbezogenen Funktionen arbeiten. Möglicherweise siehst du Pakete wie:
...
- Du dich in redundanten oder widersprüchlichen Paketen wie `user.utils.profile` vs. `user.profile.utils` verhedderst, ohne klare Zuständigkeit oder Verantwortung.

In solchen Fällen werden generische Paketnamen wie `utils`, `model` oder `dto` zu bedeutungslosen Labels. Anstatt dir zu helfen, Code basierend auf seiner Verantwortung zu finden, zwingen sie dich dazu, dich ausschließlich auf die Terminologie zu verlassen – und da verschiedene Personen oder Teams diese Begriffe oft unterschiedlich verwenden, kann dieses Vokabular sich im Laufe der Zeit ändern oder widersprechen. Dies macht das Verständnis der Codebasis stärker davon abhängig, ein sich ständig änderndes und mehrdeutiges Glossar zu beherrschen, anstatt auf intuitiven architektonischen Grenzen.
...
Lass uns ein Beispiel erstellen:
```
com.example.user
 ├─ User.kt
 ├─ UserId.kt
...
Als zusätzliche Denkweise kannst du ein "Aggregat" aus DDD als mentales Modell zur Identifizierung eines kohärenten Konzepts betrachten, das einen eigenen Namespace verdient. Die Idee ist, dass ein Paket etwas darstellen sollte, das für die Außenwelt eine eigene Bedeutung und Nützlichkeit hat.
...
Wenn dein Code beispielsweise um die Verwaltung eines einzelnen `Order` – seiner Validierung, seines Lebenszyklus und seiner Operationen – herum aufgebaut ist, ist die Benennung des Pakets `orders` irreführend. Der Plural impliziert eine Sammlung, was nicht die Tatsache widerspiegelt, dass es in dem Paket um das **Konzept Order** selbst geht. Die richtige Wahl ist `order`, was kommuniziert, dass dieser Namespace das Domänenkonzept betrifft und nicht eine Liste von Orders.

Benenne Pakete auch nicht `errors`, `events` oder `notifications`, nur weil sie mehrere Elemente dieses Typs enthalten. Die Frage, die du stellen musst, lautet: Welches **Konzept oder welche Verantwortung** erfasst dieses Paket? Benenne es nach diesem Konzept, nicht nach der Anzahl der Instanzen, die es enthält. Dies hält deine Paketnamen **präzise, aussagekräftig und auf das mentale Modell deines Systems abgestimmt**.
...
Dieser Unterschied wird wichtig, wenn das System wächst. Nicht jedes Konzept oder jede Funktion wird eine `.domain`, eine `.presentation` (zum Beispiel hat die Autorisierung höchstwahrscheinlich keine Geschäftslogik, verdient daher keine `.domain`) oder eine andere von dir erwartete Schicht haben. Das bedeutet, dass die Navigation schnell umständlich wird: Man kann nicht sagen, was eine Funktion _tut_ oder _enthält_, sondern nur, dass sie irgendwo unter einem technischen Label liegen könnte. Das Ergebnis sind ungleichmäßige Hierarchien und Ordner, die mit Schichten überladen wirken, während die eigentlichen Geschäftskonzepte vergraben werden.

Module zeigen das gleiche Muster. Im kleinen Maßstab mögen `:domain:user` und `:domain:task` in Ordnung aussehen. Aber sobald du `:application:auth`, `:application:user`, `:application:task` hinzufügst (während `:domain:auth` nicht existiert), wird die Navigation seltsam. Man kann die tatsächliche Fähigkeit einer Funktion oder eines abgegrenzten Kontexts nicht sofort erkennen: Hat `auth` überhaupt eine Domänenlogik, oder ist es nur Anwendungscode? Die Struktur betont zuerst technische Grenzen, während die konzeptuellen Grenzen unklar bleiben.

Ein konzeptzentrierter Ansatz – `com.example.user.domain`, `com.example.order.infrastructure` – vermeidet dieses Problem. Du beginnst immer mit dem, was dem Unternehmen wichtig ist, und verfeinerst es erst dann nach der Schicht.

Hat es eine `domain`- oder `infrastructure`-Schicht? Das spielt keine Rolle – wichtig ist das Konzept selbst, und erst dann, wie es intern implementiert wird. **Dies ist besonders wichtig, da sich die Struktur wahrscheinlich im Laufe der Zeit ändern wird.** Und was tust du dann?
## Fazit
...
Auf Makroebene ist es völlig in Ordnung, Pakete wie `domain` oder `infrastructure` zu haben, die die architektonische Schicht widerspiegeln, in der du arbeitest. Lege außerdem deine Prioritäten richtig fest – das Konzept steht an erster Stelle, dann die Ebene, auf der du operierst.

Verwende schließlich **standardmäßig Singularnamen**, es sei denn, das Konzept selbst ist inhärent plural, wie `news`.
...
Insgesamt kannst du diese Regeln auf andere Sprachen außerhalb des JVM-Ökosystems anwenden, wie z. B. TypeScript mit seinen Modulsystemen oder alles andere, das Namespacing als Konzept hat.
